...
{
	g_hQuestions = CreateTimer(900.0, SendQuestions, _, TIMER_REPEAT);
}	
...
public Action QuestionStatus(Handle Timer)
{
	...
	{
		canRespond = false;
		CPrintToChatAll("%s Fin de la question, personne n'as répondu.", TEAM);
	}	
}		
...
public Action SendQuestions(Handle Timer)
{
	...
	CPrintToChatAll("%s %s", TEAM, question);
...
public Action rp_SayOnPublic(int client, const char[] arg, const char[] Cmd, int args)
{
	if(canRespond)
	{
		if(StrEqual(arg, reponse))
		{
			canRespond = false;
			CPrintToChatAll("%s %N a répondu correctement à la question\nLa réponse était: %s.", TEAM, client, reponse);
			CPrintToChat(client, "%s Vous avez reçu %i$ pour votre bonne réponse.", TEAM, reward);
