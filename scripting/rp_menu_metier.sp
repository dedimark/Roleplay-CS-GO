...
{
	rp_LoadTranslation();
	g_OnMenuMetier = new GlobalForward("RP_OnPlayerMetier", ET_Event, Param_Cell, Param_Cell);
	g_HandleOnMenuMetier = new GlobalForward("RP_OnPlayerMetierHandle", ET_Event, Param_Cell, Param_String);
}
	...
	if (action == MenuAction_Select)
	{
		...
		else if (StrEqual(info, "salaire"))
		{
			...
			for (int i = 1; i <= GetMaxGrades(rp_GetClientInt(client, i_Job)); i++)
			{
				...
				char gradeName[24];
				GetGradeName(i, rp_GetClientInt(client, i_Job), STRING(gradeName));
			...
			MenuChangePrice(client);
		else if (StrEqual(info, "demission"))
		{
			char gradeName[16];
			GetGradeName(rp_GetClientInt(client, i_Grade), rp_GetClientInt(client, i_Job), STRING(gradeName));
	...
	if(action == MenuAction_Select)
	{
		...
		else if(StrEqual(info, "contrat"))
		{
			...
			if(query && rowCount != 0)
			{
				...
				{
					char name[32], strSteam[64], strJoueur[128], gradeName[16], strMenu[64];
					if(query.FetchRow())
					{
						int grade = query.FetchInt(2);						
						GetGradeName(grade, rp_GetClientInt(client, i_Job), STRING(gradeName));
...
{
	char jobNameAim[32], jobNameClient[32], strInfo[32]; 
	GetJobName(rp_GetClientInt(aim, i_Job), STRING(jobNameAim));
	GetJobName(rp_GetClientInt(client, i_Job), STRING(jobNameClient));
	...
	if(action == MenuAction_Select)
	{
		...
		if(!StrEqual(buffer[2], "rang"))
		{
			...
			else
			{
				...
				UpdateSQL(rp_GetDatabase(), "UPDATE rp_jobs SET jobid = %i, gradeid = %i WHERE steamid = '%s';", id, grade, buffer[0]);	
			}
			rp_SetClientBool(client, b_menuOpen, false);
		}
	}
		...
		if(IsValidEntity(patron))
		{
			if(StrEqual(buffer[0], "oui"))
			{
				if(rp_GetClientInt(client, i_Job) == 0)
				{
					...
					char jobName[32], gradeName[16]; 
					GetJobName(rp_GetClientInt(client, i_Job), STRING(jobName));
					GetGradeName(rp_GetClientInt(client, i_Grade), rp_GetClientInt(client, i_Job), STRING(gradeName));
...
Menu MenuChangePrice(int client)
{
	rp_SetClientBool(client, b_menuOpen, true);
	Menu menu = new Menu(Handle_ChangePrice1);
	menu.SetTitle("Choisissez le prix à modifier :");
	...
	for (int i = 0; i <= MAXITEMS; i++)
	{
		...
		char strJob[10];
		IntToString(rp_GetClientInt(client, i_Job), STRING(strJob));
		
		if(StrEqual(item_jobid, strJob))
		{
			char item_string[10];
			...
			menu.AddItem(item_string, item_name);
		}	
	}
...
public int Handle_ChangePrice1(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select)
	{
		char info[10];
		...
		char item_name[64];
		rp_GetItemData(StringToInt(info), item_type_name, STRING(item_name));
		
		char item_price[64];
		rp_GetItemData(StringToInt(info), item_type_prix, STRING(item_price));
		
		rp_SetClientBool(client, b_menuOpen, true);
		Menu menu1 = new Menu(Handle_ChangePrice2);
		menu1.SetTitle("%s (%s$) :", item_name, item_price);
		
		char strFormat[32];
		Format(STRING(strFormat), "+|%i", StringToInt(info));
		menu1.AddItem(strFormat, "Augmenter le prix");
		Format(STRING(strFormat), "-|%i", StringToInt(info));
		menu1.AddItem(strFormat, "Diminuer le prix");
...
public int Handle_ChangePrice2(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select)
	{
		char info[10], buffer[2][10];
		menu.GetItem(param, STRING(info));
		ExplodeString(info, "|", buffer, 2, 10);
		
		int itemID = StringToInt(buffer[1]);
		
		if (StrEqual(buffer[0], "+"))
			MenuModifierPrixPlus(client, itemID);
		else if (StrEqual(buffer[0], "-"))
			MenuModifierPrixMoins(client, itemID);
	}
	else if(action == MenuAction_Cancel)
	{
		if(param == MenuCancel_Exit || param == MenuCancel_ExitBack)
...
Menu MenuModifierPrixMoins(int client, int itemID)
{
	char item_name[64];
	rp_GetItemData(itemID, item_type_name, STRING(item_name));
	
	char item_price[64];
	rp_GetItemData(itemID, item_type_prix, STRING(item_price));
	int prix = StringToInt(item_price);
	
	rp_SetClientBool(client, b_menuOpen, true);
	Menu menu = new Menu(Handle_ChangePrice3);
	menu.SetTitle("%s (%s$) :", item_name, item_price);
	...
	if (min < prix - 1)
	{
		Format(strFormat, sizeof(strFormat), "-1|%i", itemID);
		menu.AddItem(strFormat, "Diminuer de 1$");
	}
	if (min < prix - 2)
	{
		Format(strFormat, sizeof(strFormat), "-2|%i", itemID);
		menu.AddItem(strFormat, "Diminuer de 2$");
	}
	if (min < prix - 5)
	{
		Format(strFormat, sizeof(strFormat), "-5|%i", itemID);
		menu.AddItem(strFormat, "Diminuer de 5$");
	}
	if (min < prix - 10)
	{
		Format(strFormat, sizeof(strFormat), "-10|%i", itemID);
		menu.AddItem(strFormat, "Diminuer de 10$");
	}
	if (min < prix - 30)
	{
		Format(strFormat, sizeof(strFormat), "-30|%i", itemID);
		menu.AddItem(strFormat, "Diminuer de 30$");
	}
	if (min < prix - 50)
	{
		Format(strFormat, sizeof(strFormat), "-50|%i", itemID);
		menu.AddItem(strFormat, "Diminuer de 50$");
	}
	if (min < prix - 100)
	{
		Format(strFormat, sizeof(strFormat), "-100|%i", itemID);
		menu.AddItem(strFormat, "Diminuer de 100$");
	}
	if (min >= prix)
	{
		Format(strFormat, sizeof(strFormat), "Le prix minimum est déjà atteint (%i$).", min);
...
Menu MenuModifierPrixPlus(int client, int itemID)
{
	char item_name[64];
	rp_GetItemData(itemID, item_type_name, STRING(item_name));
	...
	if (max > prix + 1)
	{
		...
		menu.AddItem(strFormat, "Augmenter de 1$");
	}
	if (max > prix + 2)
	{
		Format(STRING(strFormat), "2|%i", itemID);
		menu.AddItem(strFormat, "Augmenter de 2$");
	}
	if (max > prix + 5)
	{
		Format(STRING(strFormat), "5|%i", itemID);
		menu.AddItem(strFormat, "Augmenter de 5$");
	}
	if (max > prix + 10)
	{
		Format(STRING(strFormat), "10|%i", itemID);
		menu.AddItem(strFormat, "Augmenter de 10$");
	}
	if (max > prix + 30)
	{
		Format(STRING(strFormat), "30|%i", itemID);
		menu.AddItem(strFormat, "Augmenter de 30$");
	}
	if (max > prix + 50)
	{
		Format(STRING(strFormat), "50|%i", itemID);
		menu.AddItem(strFormat, "Augmenter de 50$");
	}
	if (max > prix + 100)
	{
		Format(STRING(strFormat), "100|%i", itemID);
		menu.AddItem(strFormat, "Augmenter de 100$");
	}
	if (max <= prix)
	{
		Format(STRING(strFormat), "Le prix maximum est déjà atteint (%i$).", max);
		menu.AddItem("", strFormat, ITEMDRAW_DISABLED);
	}
...
public int Handle_ChangePrice3(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select)
	{
		char info[10], buffer[2][10];
		...
		int priceToAdd = StringToInt(buffer[0]);
		int itemID = StringToInt(buffer[1]);
		...
		prix += priceToAdd;		
		Format(STRING(item_price), "%i", prix);
		rp_SetItemData(itemID, item_type_prix, STRING(item_price));
		PrintHintText(client, "Le prix est maintenant %i$.", prix);
	}
	else if(action == MenuAction_Cancel)
	{
		if(param == MenuCancel_Exit || param == MenuCancel_ExitBack)
