...
public void OnPluginStart()
{
	...
	{
		g_MenuVendre = new GlobalForward("rp_MenuVendre", ET_Event, Param_Cell, Param_Cell, Param_Cell);
		g_Handle_MenuVendre = new GlobalForward("rp_HandleMenuVendre", ET_Event, Param_Cell, Param_String);
		
		g_PushInteractionJoueur = new GlobalForward("rp_PushToInteraction", ET_Event, Param_Cell, Param_Cell);
		g_Handle_PushInteractionJoueur = new GlobalForward("rp_Handle_PushToInteraction", ET_Event, Param_Cell, Param_String);
		
		PrintToServer("[Roleplay] Vente : Licence Valide");
	}
	else
		UnloadPlugin();
}
...
public Action rp_OnClientInteract(int client, int aim, const char[] entName, const char[] entModel, const char[] entClassName)
{
	if(StrContains(entClassName, "player") != -1)
	{
		if(rp_GetClientInt(client, i_Job) != 0)
		{
			...
			menu.SetTitle("Intéraction avec %N", aim);
			Format(STRING(strIndex), "vendre|%i", aim);
