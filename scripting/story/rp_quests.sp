...
{	
	...
	rp_GetDatabase().Query(CallBack, buffer, GetClientUserId(client));
}
...
{	
	...
	if(!Results.FetchRow()) 
	{
		...
		{
			rp_SetClientBool(client, b_isClientNew, true);	
			rp_SetClientBool(client, b_isInvincible, true);				
			Q1_Frame(client);	
	...
	{
		rp_SetClientBool(client, b_IsNoPyj, view_as<bool>(SQL_FetchIntByName(Results, "nopyj")));
	}	
...
{
	TeleportEntity(client, view_as<float>({1070.300659, -74.285339, -2112.030029}), NULL_VECTOR, NULL_VECTOR);
...
public int MenuSelectJob(Menu menu, MenuAction action, int client, int param2) 
{
	if(action == MenuAction_Select) 
	{
		...
		rp_ClientGiveItem(client, i_plante, 1);
		rp_ClientGiveItem(client, i_imprimantes, 1);
		...
		UpdateSQL(rp_GetDatabase(), "INSERT IGNORE INTO `rp_logs` (`Id`, `steamid`, `playername`, `ip`, `serial`, `nopyj`, `timestamp`) VALUES (NULL, '%s', '%s', '%s', '%i', '0', CURRENT_TIMESTAMP);", steamID[client], clean_playername, playerIP[client], GetClientSerial(client));
