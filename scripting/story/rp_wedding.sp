...
public void OnPluginStart() 
{
	...
	BuildPath(Path_SM, STRING(logFile), "logs/roleplay/rp_wedding.log");
...
public void OnClientPostAdminCheck(int client) 
{
	...
	rp_GetDatabase().Query(SQL_CheckWedding, query, GetClientUserId(client));
}
...
{	
	...
	while (Results.FetchRow()) 
	{
		...
		LoopClients(i) 
		{
			...
			{
				CPrintToChat(i, "%s Votre conjoint %N a rejoint la ville.", TEAM, client);
				CPrintToChat(client, "%s Votre conjoint %N a rejoint la ville.", TEAM, i);
...
public void OnClientDisconnect(int client) 
{
	...
	if( mari > 0 ) 
	{
		CPrintToChat(mari, "%s Votre conjoint a quitté la ville précipitamment.", TEAM);
...
{	
	...
	else if(rp_GetClientInt(client, i_ByteZone) != 7) 
	{
		CPrintToChat(client, "%s Cette commande ne peut être utilisée qu'au tribunal.", TEAM);
...
{	
	int pos_1 = rp_GetClientInt(juge, i_ByteZone);
	int prix = 10000;
	...
	{
		PrintToChatZone(pos_1, "%s L'un des mariés est en fait un SDF refoulé et n'a pas assez d'argent pour s'acquitter des frais du mariage, vous pouvez huer les pauvres !", TEAM);
		return;
	}
	
	PrintToChatZone(pos_1, "%s %N répond: OUI !", TEAM, epouse);
	PrintToChatZone(pos_1, "%s %N et %N sont maintenant unis par les liens du mariage, vous pouvez féliciter les mariés !", TEAM, epoux, epouse);
	
	CPrintToChat(epoux, "%s Vous et %N êtes unis par les liens du mariage, vous pouvez embrasser la mariée, félicitation !", TEAM, epouse);
	CPrintToChat(epouse, "%s Vous et %N êtes unis par les liens du mariage, félicitations !", TEAM, epoux);
	...
	char query[512];
	Format(STRING(query), "INSERT INTO `rp_wedding` (`epouxID`, `epousseID`, `timestamp`) VALUES ('%s', '%s', CURRENT_TIMESTAMP);", steamID[epoux], steamID[epouse]);
	rp_GetDatabase().Query(SQLErrorCheckCallback, query);
...
{
	int zone = rp_GetClientInt(client, i_ByteZone);
	...
	if(b == 0) 
	{
		...
		LoopClients(i) 
		{
			if(!IsClientValid(i) || rp_GetClientInt(i, i_ByteZone) != zone)
				continue;
	...
	else if(c == 0) 
	{
		...
		LoopClients(i) 
		{
			if( !IsClientValid(i) || b == i || rp_GetClientInt(i, i_ByteZone) != zone )
				continue;
			if( rp_GetClientInt(i, i_MarriedTo) > 0 )
				continue;
	...
	else if(d == 0) 
	{
		...
		rp_GetDatabase().Query( SQL_CheckWedding2, query, dp);
	}
	else if(d > 0) 
	{
		if(d >= 2) 
		{
			CPrintToChat(a, "%s Vous essayez d'unir quelqu'un déjà marié, le mariage ne peut pas se dérouler.", TEAM);
		}
		else 
		{
			int pos_1 = rp_GetClientInt(a, i_ByteZone);
			int pos_2 = rp_GetClientInt(b, i_ByteZone);
			int pos_3 = rp_GetClientInt(c, i_ByteZone);
			int err = 0;
			
			// Messages d'erreurs double check
			if(pos_1 != 7) 
			{
				CPrintToChat(a, "%s Vous n'êtes pas au tribunal, le mariage ne peut pas se dérouler.", TEAM);
				err++;
			}
			if(pos_1 != pos_3 || pos_2 != pos_3) 
			{
				CPrintToChat(a, "%s Tous les prétendants ne sont pas dans la même salle du tribunal, le mariage ne peut pas se dérouler.", TEAM);
				err++;
			}
			...
			if(err == 0) 
			{
				PrintToChatZone(pos_1, "%s Le juge %N s'exclame: %N, voulez-vous prendre pour époux %N et l'aimer jusqu'à ce que la mort vous sépare?", TEAM, a, b, c);
	...
	else if( e > 0 ) 
	{
		...
		if( e == 2 ) 
		{
			PrintToChatZone(pos_1, "%s %N répond: NON, %N fond en larmes... Stupéfaction dans la salle .", TEAM, b, c);
		}
		else 
		{			
			// Messages à toute la salle
			PrintToChatZone(pos_1, "%s %N répond: OUI, les invités et %N sourient .", TEAM, b, c);
			PrintToChatZone(pos_1, "%s Le juge %N s'exclame: %N, voulez-vous prendre pour épouse %N et l'aimer jusqu'à que la mort vous sépare?", TEAM, a, c, b);
	...
	{		
		...
		if( e == 2 ) 
		{
			PrintToChatZone(pos_1, "%s %N répond: NON, %N fond en larmes... Stupéfaction dans la salle .", TEAM, c, b);
		}
		else 
		{
			...
			if( pos_1 != pos_3 || pos_2 != pos_3 ) 
			{
				CPrintToChat(a, "%s Tous les prétendant ne sont pas dans la même salle du tribunal, le mariage ne peut pas se dérouler.", TEAM);
				err++;
			}
			if( err == 0 )
				Marier(a, b, c);
...
Menu Menu_Prolonge(int& client, int a, int b, int c, int d, int e) 
{
	...
	if( b == 0 ) 
	{
		...
		LoopClients(i) 
		{
			...
			if( to > 0 && i < to && rp_GetClientInt(i, i_ByteZone) == zone && rp_GetClientInt(to, i_ByteZone) == zone ) {
	...
	else if( d > 0 ) 
	{
		...
		{
			PrintToChatZone(zone, "%s %N ne souhaite pas prolonger son marriage.", TEAM, b);
		}
	}
	else if( e > 0 ) 
	{
		if( e == 1 ) 
		{
			...
			{
				PrintToChatZone(zone, "%s L'un des mariés n'a pas assez d'argent pour prolonger son contrat de mariage.", TEAM);
				return null;
			}
			
			PrintToChatZone(zone, "%s Le contrat de mariage de %N et %N est prolongé de 7 jours!", TEAM, b, c);
		...
		{
			PrintToChatZone(zone, "%s %N ne souhaite pas prolonger son mariage.", TEAM, c);
		}
	}
	return subMenu;
}
...
Menu Menu_Divorce(int& client, int a, int b, int c) 
{
	...
	if( b == 0 ) 
	{
		for (int i = 1; i <= MaxClients; i++) 
		{
			if( !IsClientValid(i) )
				continue;
			if( rp_GetClientInt(i, i_ByteZone) == zone ) 
			{
		...
		rp_GetDatabase().Query(SQL_CheckDivorce, query, GetClientUserId(client));
	}
	else if( c == 0 ) 
	{
	...
	else 
	{
		if( c == 1 ) 
		{
			...
			{
				PrintToChatZone(zone, "%s %N n'a pas assez d'argent pour prolonger son contrat de mariage.", TEAM, b);
				return null;
			}
			
			PrintToChatZone(zone, "%s %N a rompu son contrat de mariage!", TEAM, b);
			...
			{
				CPrintToChat(rp_GetClientInt(b, i_MarriedTo), "%s Votre conjoint a rompu votre contrat de mariage.", TEAM);
		...
		{
			PrintToChatZone(zone, "%s %N ne veut pas rompre son contrat de mariage.", TEAM, b);
		}
	}
	return subMenu;
}
...
Menu Menu_Duration(int client, int a, int b) 
{
	...
	{	
		...
		rp_GetDatabase().Query(SQL_CheckStatus, query, GetClientUserId(client));
	}
	else {
		float j = b / (24.0 * 60.0 * 60.0);
		PrintToChatZone(zone, "%s %N est toujours marié pour une durée de %.1f jour%s.", TEAM, a, j, j >= 2.0 ? "s" : "");
	}
