...
public Action rp_OnClientTakeDamage(int victim, int attacker, int dmg_health, int dmg_armor, const char[] weaponName)
{
	if(rp_GetEventType() == event_type_murder)
	{
		...
		&& attacker > 0)
		{
			...
			|| countItem[attacker] >= 3)
			{
				...
				CPrintToChat(victim, "%s Vous avez été tué par un innocent.", TEAM);
