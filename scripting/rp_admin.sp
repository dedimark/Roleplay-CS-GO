...
public void RP_OnDatabaseLoaded(Database db)
{
	...
	"CREATE TABLE IF NOT EXISTS `rp_admins` ( \
...
public void LoadAdmin(int client) 
{
	...
	char buffer[512];
	Format(STRING(buffer), "SELECT * FROM rp_admins WHERE steamid = '%s'", steamID[client]);
...
{	
	...
	{
		rp_SetClientInt(client, i_AdminLevel, SQL_FetchIntByName(Results, "level"));		
		char rank[64];
		SQL_FetchStringByName(Results, "tag", STRING(rank));
...
Menu MenuMute(int client)
{
	...
	menu.SetTitle("Choisissez le mute à attribuer");
	menu.AddItem("global", "Tchat Global");
	menu.AddItem("local", "Tchat Local");
	menu.AddItem("voice", "Vocal");
	menu.ExitButton = true;
	menu.Display(client, MENU_TIME_FOREVER);
}
...
public int DoMenuSetJobSub(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select)
	{
		...
		if(numeroJob != 0)
		{
			...
			char strMenu[32];		
			for (int i = 1; i <= GetMaxGrades(numeroJob); i++)
			{
				char gradeName[16];
				GetGradeName(i, numeroJob, STRING(gradeName));
				
				Format(STRING(strMenu), "%i|%i|%i", numeroJob, i, joueur);
				SetJobSub2.AddItem(strMenu, gradeName);
			}	
		...
		{
			char oldJob[32];
			GetJobName(rp_GetClientInt(joueur, i_Job), STRING(oldJob));
			
			rp_SetClientInt(joueur, i_Job, 0);
			rp_SetClientInt(joueur, i_Grade, 0);
			
			char newJob[32];
			GetJobName(rp_GetClientInt(joueur, i_Job), STRING(newJob));
			...
			Embed.SetColor("#00fd29");
			Embed.SetTitle(hostname);
			Embed.SetTitleLink("steam://connect/54.36.62.67:27015");
			if(IsClientValid(client) && IsClientValid(joueur))
			{
				char message[128];
				Format(STRING(message), "%N a mis %N %s", client, joueur, newJob);			
			...
			LoadSalaire(joueur);			
			UpdateSQL(rp_GetDatabase(), "UPDATE rp_jobs SET jobid = %i, gradeid = %i WHERE steamid = '%s';", 0, 0, steamID[joueur]);
...
public int DoMenuSetJobFinal(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select)
	{
		...
		char oldJobName[32], oldGrade[16], oldJob[32];	
		GetJobName(rp_GetClientInt(joueur, i_Job), STRING(oldJobName));
		GetGradeName(rp_GetClientInt(joueur, i_Grade), rp_GetClientInt(joueur, i_Job), STRING(oldGrade));
		...
		if(numeroJob != 0)
		{
			...
			UpdateSQL(rp_GetDatabase(), "UPDATE rp_jobs SET jobid = %i, gradeid = %i WHERE steamid = '%s';", numeroJob, numeroGrade, steamID[joueur]);
		}
		else
		{
			rp_SetClientInt(joueur, i_Job, 0);
		...
		GetJobName(rp_GetClientInt(joueur, i_Job), STRING(jobName));
		GetGradeName(rp_GetClientInt(joueur, i_Grade), rp_GetClientInt(joueur, i_Job), STRING(gradeName));
		...
		{
			char message[128];
			Format(STRING(message), "%N a mis %N (%s %s)", client, joueur, gradeName, jobName);			
			Embed.AddField("Message", message, false);
		}	
		Embed.SetFooter("Roleplay CS:GO | VR-HOSTING.FR");
		Embed.SetFooterIcon("https://steamcdn-a.akamaihd.net/steamcommunity/public/images/avatars/2c/2cf89047920724a188854e85a1e7056d78a05d9e_full.jpg");
...
public int DoMenuMute(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select)
	{
		char info[64], strIndex[128], strName[64];
		menu.GetItem(param, STRING(info));
		
		rp_SetClientBool(client, b_menuOpen, true);
		Menu menu1 = new Menu(DoMenuMenu_Final);
		menu1.SetTitle("Qui voulez-vous muter ?");
		
		LoopClients(i)
		{
			if(!IsPlayerAlive(i))
			{
				Format(STRING(strName), "%N [mort]", i);
				menu1.AddItem("", strName, ITEMDRAW_DISABLED);
			}	
			else
			{
				Format(STRING(strIndex), "%i|%s", i, info);
				Format(STRING(strName), "%N", i);
				menu1.AddItem(strIndex, strName);	
			}
		}	
...
public int DoMenuMenu_Final(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select)
	{
		char info[128], buffer[2][128], strCmd[128];
		menu.GetItem(param, STRING(info));
		ExplodeString(info, "|", buffer, 2, 128);
		
				
		/*if(String_IsNumeric(buffer[0]))
			strcopy(STRING(buffer[0]), steamID[StringToInt(buffer[0])]);*/
		
		Format(STRING(strCmd), "rp_mute %i %s", StringToInt(buffer[0]), buffer[1]);
		ClientCommand(client, strCmd);
		MenuMute(client);
	}
	else if(action == MenuAction_Cancel)
...
public int DoMenuGererAdminFinal(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select)
	{
		...
		SetSQL_Int(rp_GetDatabase(), "rp_admin", "adminid", StringToInt(buffer[1]), steamID[joueur]);
...
public Action Command_Info(int client, int args)
{
	...
	if(aim <= MaxClients)
	{
		...
		CPrintToChat(client, "{darkred}************{orange}MALADIES{darkred}************");		
		if(rp_GetClientBool(aim, b_asMaladie_Covid))
			CPrintToChat(client, "{darkred}* {green} Covid-19");
		if(rp_GetClientBool(aim, b_asMaladie_Diarree))
			CPrintToChat(client, "{darkred}* {green} Diarree");
		if(rp_GetClientBool(aim, b_asMaladie_Angine))
			CPrintToChat(client, "{darkred}* {green} Angine");
		if(rp_GetClientBool(aim, b_asMaladie_Peste))
			CPrintToChat(client, "{darkred}* {green} Peste");		
		CPrintToChat(client, "{darkred}*********************************");		

		CPrintToChat(client, "{darkred}************{orange}CHIRURGIES{darkred}************");		
		if(rp_GetClientBool(aim, b_asChiru_Coeur))
			CPrintToChat(client, "{darkred}* {green} Coeur");
		if(rp_GetClientBool(aim, b_asChiru_Jambe))
			CPrintToChat(client, "{darkred}* {green} Jambes");
		if(rp_GetClientBool(aim, b_asChiru_Foie))
			CPrintToChat(client, "{darkred}* {green} Foie");
		if(rp_GetClientBool(aim, b_asChiru_Poumon))
			CPrintToChat(client, "{darkred}* {green} Poumons");
		if(rp_GetClientBool(aim, b_asChiru_Muscle))
			CPrintToChat(client, "{darkred}* {green} Muscles");		
		CPrintToChat(client, "{darkred}*********************************");	
	}
...
public Action Command_DBSkin(int client, int args)
{
	...
	LoopClients(i) 
	{
		if(StrEqual(arg1, steamID[i]))
		{
			...
			UpdateSQL(rp_GetDatabase(), "UPDATE rp_vetements SET skin = '%s' WHERE steamid = '%s';", arg2, steamID[joueur]);
...
public Action Command_Respawn(int client, int args)
{
	...
	LoopClients(i)
	{
		if(IsClientValid(joueur[i]))
		{
			...
			{
				CS_RespawnPlayer(i);
				CPrintToChat(i, "%s {lightred}[ADMIN]{default} Vous avez été respawn.", TEAM);
...
public Action Command_Mute(int client, int args)
{
	...
	{
		if(client > 0)
			CPrintToChat(client, "%s Usage : rp_mute <joueur> <type>", TEAM);
		else
			PrintToServer("[ADMIN] Usage : rp_mute <joueur> <type>");
		return Plugin_Handled;
	}
	...
	int joueur = StringToInt(arg1);

	if(IsClientValid(joueur))
	{
		if(StrEqual(arg2, "global"))
		{
			if(rp_GetClientBool(joueur, b_IsMuteGlobal))
			{
				rp_SetClientBool(joueur, b_IsMuteGlobal, false);
				PrintHintText(joueur, "Vous n'êtes plus muet [CHAT GLOBAL].");
				CPrintToChat(joueur, "%s Vous n'êtes plus muet {lightgreen}[CHAT GLOBAL]{default}.", TEAM);
			}
			else
			{
				rp_SetClientBool(joueur, b_IsMuteGlobal, true);
				PrintHintText(joueur, "Vous êtes muet [CHAT GLOBAL].");
				CPrintToChat(joueur, "%s Vous êtes muet {lightred}[CHAT GLOBAL]{default}.", TEAM);
			}
			...
			if(joueur != client)
			{
				...
				{
					if(rp_GetClientBool(joueur, b_IsMuteGlobal))
						CPrintToChat(client, "%s {yellow}%N {default}est muet {lightred}[CHAT GLOBAL]{default}.", TEAM, joueur);
					else
						CPrintToChat(client, "%s {yellow}%N {default}n'est plus muet {lightgreen}[CHAT GLOBAL]{default}.", TEAM, joueur);
				}
				else
				{
					if(rp_GetClientBool(joueur, b_IsMuteGlobal))
						PrintToServer("[ADMIN] %N est muet [CHAT GLOBAL].", joueur);
					else
						PrintToServer("[ADMIN] %N n'est plus muet [CHAT GLOBAL].", joueur);
				}
			}
		}
		else if(StrEqual(arg2, "local"))
		{
			if(rp_GetClientBool(joueur, b_IsMuteLocal))
			{
				rp_SetClientBool(joueur, b_IsMuteLocal, false);
				PrintHintText(joueur, "Vous n'êtes plus muet [CHAT LOCAL].");
				CPrintToChat(joueur, "%s Vous n'êtes plus muet {lightgreen}[CHAT LOCAL]{default}.", TEAM);
			}
			else
			{
				rp_SetClientBool(joueur, b_IsMuteLocal, true);
				PrintHintText(joueur, "Vous êtes muet [CHAT LOCAL].");
				CPrintToChat(joueur, "%s Vous êtes muet {lightred}[CHAT LOCAL]{default}.", TEAM);
			}
			...
			if(joueur != client)
			{
				...
				{
					if(rp_GetClientBool(joueur, b_IsMuteLocal))
						CPrintToChat(client, "%s {yellow}%N {default}est muet {lightred}[CHAT LOCAL]{default}.", TEAM, joueur);
					else
						CPrintToChat(client, "%s {yellow}%N {default}n'est plus muet {lightgreen}[CHAT LOCAL]{default}.", TEAM, joueur);
				}
				else
				{
					if(rp_GetClientBool(joueur, b_IsMuteLocal))
						PrintToServer("[ADMIN] %N est muet [CHAT LOCAL].", joueur);
					else
						PrintToServer("[ADMIN] %N n'est plus muet [CHAT LOCAL].", joueur);
				}
			}
		}	
		if(StrEqual(arg2, "voice"))
		{
			if(rp_GetClientBool(joueur, b_IsMuteVocal))
			{
				rp_SetClientBool(joueur, b_IsMuteVocal, false);
				PrintHintText(joueur, "Vous n'êtes plus muet [VOCAL].");
				CPrintToChat(joueur, "%s Vous n'êtes plus muet {lightgreen}[VOCAL]{default}.", TEAM);
			}
			else
			{
				rp_SetClientBool(joueur, b_IsMuteVocal, true);
				PrintHintText(joueur, "Vous êtes muet [VOCAL].");
				CPrintToChat(joueur, "%s Vous êtes muet {lightred}[VOCAL]{default}.", TEAM);
			}
			...
			if(joueur != client)
			{
				...
				{
					if(rp_GetClientBool(joueur, b_IsMuteVocal))
						CPrintToChat(client, "%s {yellow}%N {default}est muet {lightred}[VOCAL]{default}.", TEAM, joueur);
					else
						CPrintToChat(client, "%s {yellow}%N {default}n'est plus muet {lightgreen}[VOCAL]{default}.", TEAM, joueur);
				}
				else
				{
					if(rp_GetClientBool(joueur, b_IsMuteVocal))
						PrintToServer("[ADMIN] %N est muet [VOCAL].", joueur);
					else
						PrintToServer("[ADMIN] %N n'est plus muet [VOCAL].", joueur);
				}
			}
		}			
	}
