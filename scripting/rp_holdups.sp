...
int g_iPlayerTeam[2049], g_stkTeam[QUEST_TEAMS + 1][MAXPLAYERS + 1], g_stkTeamCount[QUEST_TEAMS + 1], g_iMaskEntity[MAXPLAYERS + 1];
	...
	if (IsClientValid(client))
	{
		if (rp_GetClientInt(client, i_Job) == 2)
		{
				...
				if (!g_bHoldup)
				{
					...
					if (CTCount >= GetConVarInt(HoldupNeededCT))
					{
						...
						if(rp_GetClientInt(client, i_ByteZone) == 155)
							Format(STRING(zone), "%T", "Zone_McDonalds", LANG_SERVER);
						else if(rp_GetClientInt(client, i_ByteZone) == 144)
							Format(STRING(zone), "%T", "Zone_Dolce&Gabanna", LANG_SERVER);
					...
					{
						char Translation[128];
						Format(STRING(Translation), "%T", "Holdup_Requires", LANG_SERVER, GetConVarInt(HoldupNeededCT));
			...
			else
			{
				...
				CPrintToChat(client, "%s %s", Translation);
			}
		}
		else
		{
				...
				{													
					...
					LoopClients(braqueurs)
					{
						if(isComplice[braqueurs])
						{
							...
							PrintHintText(braqueurs, "%s", Translation);
						}	
					}	
				}
				else
				{
					...
					LoopClients(braqueurs)
					{
						if(isComplice[braqueurs])
						{
							...
							CPrintToChat(isComplice[braqueurs], "           		%s           		", Translation);
...
public Action Timer_Holdup_Refresh(Handle timer)
{
	...
	CPrintToChatAll("%s %s", TEAM, Translation);
}
...
public void OnClientDisconnect(int client) 
{
	if(g_iPlayerTeam[client] == TEAM_BRAQUEUR)
		...
		rp_GetDatabase().Query(SQLErrorCheckCallback, szQuery);	
	}
