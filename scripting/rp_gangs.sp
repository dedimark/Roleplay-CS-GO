...
public Action RP_OnPlayerSay(int client, const char[] arg)
{
	if(canDefine[client])
	{
		if(strlen(arg) > 64)
		{
			CPrintToChat(client, "%s Le nom du groupe est trop long ! Réessayez.", TEAM);
	...
	if(action == MenuAction_Select)
	{
		...
		else if(StrEqual(info, "left"))
		{
			if(IsClientValid(client))
			{
				...
				CPrintToChat(client, "%s Vous avez quitté %s", TEAM, groupe_name);
...
public int DoInvitationGroupFinal(Menu menu, MenuAction action, int client, int param)
{
	if(action == MenuAction_Select) 
	{
		...
		if(StrEqual(buffer[0], "accepter")) 
		{
			...
			CPrintToChat(client, "%s Vous avez accepter l'invitation de %N pour rejoindre %s.", TEAM, joueur, groupename);
			CPrintToChat(joueur, "%s %N a accepter votre invitation.", TEAM, client);
			rp_SetClientBool(client, b_menuOpen, false);
		}
		else if(StrEqual(buffer[0], "refuser")) 
		{
			CPrintToChat(client, "%s Vous avez refuser l'invitation de %N pour rejoindre %s.", TEAM, joueur, groupename);
