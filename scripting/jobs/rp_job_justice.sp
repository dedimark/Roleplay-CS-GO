...
public void OnPluginStart()
{
	...
	RegConsoleCmd("audience", Cmd_Audience);
	RegConsoleCmd("audiance", Cmd_Audience);
...
public int MenuTribunal_main(Menu menu, MenuAction action, int client, int param) 
{
	if(action == MenuAction_Select) 
	{
		...
		{		
			...
			DBResultSet hQuery = SQL_Query(rp_GetDatabase(), szQuery);
		...
		{			
			...
			DBResultSet hQuery = SQL_Query(rp_GetDatabase(), "SELECT `steamid`, `name` FROM `rp_users` ORDER BY `rp_users`.`last_connected` DESC LIMIT 100;");
			char tmp[255], tmp2[255], szSteam[32];
...
public int MenuTribunal_selectplayer(Menu menu, MenuAction action, int client, int param) 
{
	...
	{		
		...
		SQL_Query(rp_GetDatabase(), szQuery);
...
public int MenuTribunal_Apply(Menu menu, MenuAction action, int client, int param) 
{
	if(action == MenuAction_Select) 
	{
		...
		if(StringToInt(g_szTribunal_DATA[client][tribunal_duration]) > 0)
			CPrintToChat(client, "%s Afin de confirmer votre jugement, tappez maintenant /jugement amende %s raison", TEAM, random);
		else
			CPrintToChat(client, "%s Afin de confirmer votre jugement, tappez maintenant /jugement %s raison", TEAM, random);
	}
	else if (action == MenuAction_Cancel)
	{
		if (param == MenuCancel_Exit)
...
public int MenuTribunal(Menu menu, MenuAction action, int client, int param) 
{
	...
	{		
		...
		{			
			...
			{
				CPrintToChat(client, "%s Le joueur s'est déconnecté.", TEAM);
				return;
			}
...
public Action Cmd_Conv(int client, int args) 
{
	...
	rp_SetClientBool(client, b_menuOpen, true);
	Menu menu = new Menu(eventConvocation);
	...
	LoopClients(i)
	{
		...
		if(rp_GetClientBool(client, b_isClientNew))
...
public int eventConvocation(Menu menu, MenuAction action, int client, int param) 
{
	if (action == MenuAction_Select) 
	{
		char options[128];
		...
		// Setup menu
		rp_SetClientBool(client, b_menuOpen, true);
		Menu menu2 = new Menu(eventConvocation_2);
...
public int eventConvocation_2(Menu menu, MenuAction action, int client, int param) 
{
	if(action == MenuAction_Select) 
	{
		...
		if(etat == -1) 
		{
			...
			CPrintToChat(client, "%s La recherche sur le joueur %N à durée %.1f minutes.", TEAM, target, (GetTime()-g_TribunalSearch[target].tribunal_search_starttime)/60.0);
			LogToGame("%s [RECHERCHE] %L à mis fin à la convocation de %L.", TEAM, client, target);
		...
		else if(etat == 1) 
		{
			...
			CPrintToChatAll("{lightblue} ================================== {default}");
			CPrintToChatAll("[TRIBUNAL]{default} %N {default}est convoqué dans le Tribunal. [%i/3]", target, etat);
			CPrintToChatAll("{lightblue} ================================== {default}");
			LogToGame("%s [RECHERCHE] %L convoque %L au tribunal.", TEAM, client, target);
		...
		else if(etat == 4) 
		{
			...
			CPrintToChatAll("{lightblue} ================================== {default}");
			LogToGame("%s [RECHERCHE] %L à lancé une recherche sur %L", TEAM, client, target);
...
public Action Timer_ConvTribu(Handle timer, any target) 
{
	...
	if(GetVectorDistance(vecOrigin, view_as<float>({496.0, -1787.0, -1997.0})) < 64.0 || GetVectorDistance(vecOrigin, view_as<float>({-782.0, -476.0, -2000.0})) < 64.0)
	{
		...
		CPrintToChatAll("{lightblue} ================================== {default}");
		g_TribunalSearch[target].tribunal_search_status = -1;
		LogToGame("%s [RECHERCHE] %L à été détecté comme présent au tribunal.", TEAM, target);
		PrintToChatZone(rp_GetClientInt(target, i_ByteZone), "{lightblue}[TSX-RP]{default} La recherche sur le joueur %N à durée %.1f minutes.", target, (GetTime()-g_TribunalSearch[target].tribunal_search_starttime)/60.0);
		rp_SetClientBool(target, b_IsSearchByTribunal, false);
		return Plugin_Stop;
	}
...
public Action Cmd_Audience(int client, int args) 
{
	if(client == 0)
	{
		PrintToServer("%T", "Command_NoAcces", LANG_SERVER);
	...
	GetJobName(rp_GetClientInt(client, i_Job), STRING(tmp2));
	Format(STRING(tmp), "https://www.ts-x.eu/popup.php?url=https://docs.google.com/forms/d/1u4PFUsNBtVphggSyF3McU0gkA_o-6jEMSk0qmp0epFU/viewform?entry.249878658=%N%20-%20%s", client, tmp2);		
...
public void ClientConVar(QueryCookie cookie, int client, ConVarQueryResult result, const char[] cvarName, const char[] cvarValue) 
{
	if(StrEqual(cvarName, "cl_disablehtmlmotd"))
	{
		...
		{
			CPrintToChat(client, "%s Des problemes d'affichage ? Entrer cl_disablehtmlmotd 0 dans votre console puis relancer CS:GO.", TEAM);
		}
	}
}
...
public Action Cmd_Jugement(int client, int args) 
{
	...
	if(StrEqual(g_szTribunal_DATA[client][tribunal_code], arg1)) 
	{
		...
		{
			CPrintToChat(client, "%s Erreur: Pas de jugement en cours.", TEAM);
			return Plugin_Handled;
		}
		...
		if( StringToInt(g_szTribunal_DATA[client][tribunal_duration]) > 0 ) {
			...
			if(amende >= 1)
			{
				...
				if(amende > maxAmount)
				{
					CPrintToChat(client, "%s L'amende excède le montant maximum autorisé.", TEAM);
					String_GetRandom(STRING(random), sizeof(random) - 1);
				...
				if(playermoney == -1)
				{
					...
					CPrintToChat(client, "%s Erreur: Impossible de relever l'argent du joueur.", TEAM, playermoney);
				...
				{
					CPrintToChat(client, "%s Le joueur n'a que %i$, le jugement a été annulé.", TEAM, playermoney);
					String_GetRandom(random, sizeof(random), sizeof(random) - 1);
			...
			SQL_TQuery(DB, SQLErrorCheckCallback, szQuery);
			...
			LogToGame("[TRIBUNAL_V2] le juge %s %s a condamné %s à faire %s heures de prison et à payer %i$ pour %s",
				UserName,
				steamID[client],
				g_szTribunal_DATA[client][tribunal_steamid],
			...
			CPrintToChatAll("%s Le juge %s %s a condamné %s à faire %s heures de prison et à payer %i$ pour %s", TEAM, UserName, steamID[client], g_szTribunal_DATA[client][tribunal_steamid], g_szTribunal_DATA[client][tribunal_duration], amende, szReason);
		}
		else
		{
			...
			CPrintToChatAll("%s Le juge %s %s a acquitté %s pour %s", TEAM, UserName, steamID[client], g_szTribunal_DATA[client][tribunal_steamid], szReason);
		}
	...
	{
		CPrintToChat(client, "%s Le code est incorrect, le jugement a été annulé.", TEAM);
	}
