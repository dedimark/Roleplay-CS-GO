...
public void OnPluginStart()
{
	...
	BuildPath(Path_SM, logFile, sizeof(logFile), "logs/roleplay/jobs/rp_job_mercenaire.log");
}
...
public Action RP_OnPlayerInteract(int client, int target, const char[] class, const char[] model, const char[] name)
{
	...
	if(StrEqual(entityName, "coffre_mercenaire"))
		Coffre(client);	
}	
...
{	
	if(rp_GetClientInt(client, i_Job) == 2)
	{
		...
		menu.SetTitle("Coffre Mercenaire :");	
		
		menu.AddItem("", "En Developement", ITEMDRAW_DISABLED);		
