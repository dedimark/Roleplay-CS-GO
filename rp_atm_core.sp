...
#define MAX_NPCS 512
#define MAX_TYPES 64
#define _PREFIX_	"{red}[{lightblue}Roleplay{red}]{default}"
...
{
	name = "[Roleplay] Système Distributeur", 
	author = "Benito", 
...
public void OnPluginStart()
{
	...
	{
		RegConsoleCmd("sm_atm", cmdSpawnNpc, "Spawn a ATM");
		RegConsoleCmd("sm_editatm", cmdEditNpc, "Edits an ATM");	
...
public void GotDatabase(Database db, const char[] error, any data)
{
	if (db == null)
	{
		...
		Format(createTableQuery, sizeof(createTableQuery), 
		  "CREATE TABLE IF NOT EXISTS `rp_atms` ( \
...
public Action cmdEditNpc(int client, int args) {
	...
	menu.AddItem("delete", "Supprimer l' A.T.M");
...
public int editMenuHandler(Menu menu, MenuAction action, int client, int item) {
	if (action == MenuAction_Select) {
		...
		if (StrEqual(cValue, "model")) {
			...
			Format(removeNpcQuery, sizeof(removeNpcQuery), "DELETE FROM rp_atms WHERE uniqueId = '%s'", npcUniqueId);
...
public int editPositionMenuHandler(Menu menu, MenuAction action, int client, int item) {
	if (action == MenuAction_Select) {
		...
		if (StrEqual(cValue, "up")) {
			...
			Format(updatePositionQuery, sizeof(updatePositionQuery), "UPDATE rp_atms SET pos_z = '%.2f' WHERE uniqueId = '%s'", pos[2], npcUniqueId);
			...
			openPositionMenu(client);
			char updatePositionQuery[512];
			Format(updatePositionQuery, sizeof(updatePositionQuery), "UPDATE rp_atms SET pos_x = '%.2f' WHERE uniqueId = '%s'", selfPos[0], npcUniqueId);
			g_DB.Query(SQLErrorCheckCallback, updatePositionQuery);
			Format(updatePositionQuery, sizeof(updatePositionQuery), "UPDATE rp_atms SET pos_y = '%.2f' WHERE uniqueId = '%s'", selfPos[1], npcUniqueId);
			g_DB.Query(SQLErrorCheckCallback, updatePositionQuery);
			Format(updatePositionQuery, sizeof(updatePositionQuery), "UPDATE rp_atms SET pos_z = '%.2f' WHERE uniqueId = '%s'", selfPos[2], npcUniqueId);
			g_DB.Query(SQLErrorCheckCallback, updatePositionQuery);
			...
			openPositionMenu(client);
			char updatePositionQuery[512];
			Format(updatePositionQuery, sizeof(updatePositionQuery), "UPDATE rp_atms SET pos_x = '%.2f' WHERE uniqueId = '%s'", pos[0], npcUniqueId);
			g_DB.Query(SQLErrorCheckCallback, updatePositionQuery);
			...
			openPositionMenu(client);
			char updatePositionQuery[512];
			Format(updatePositionQuery, sizeof(updatePositionQuery), "UPDATE rp_atms SET pos_y = '%.2f' WHERE uniqueId = '%s'", pos[1], npcUniqueId);
			g_DB.Query(SQLErrorCheckCallback, updatePositionQuery);
...
public int editAnglesMenuHandler(Menu menu, MenuAction action, int client, int item) {
	if (action == MenuAction_Select) {
		...
		if (StrEqual(cValue, "plus")) {
			...
			Format(updateAnglesQuery, sizeof(updateAnglesQuery), "UPDATE rp_atms SET angle_y = '%.2f' WHERE uniqueId = '%s'", angles[1], npcUniqueId);
			...
			openAnglesMenu(client);
			char updateAnglesQuery[512];
			Format(updateAnglesQuery, sizeof(updateAnglesQuery), "UPDATE rp_atms SET angle_x = '%.2f' WHERE uniqueId = '%s'", selfAngles[0], npcUniqueId);
			g_DB.Query(SQLErrorCheckCallback, updateAnglesQuery);
			Format(updateAnglesQuery, sizeof(updateAnglesQuery), "UPDATE rp_atms SET angle_y = '%.2f' WHERE uniqueId = '%s'", selfAngles[1], npcUniqueId);
			g_DB.Query(SQLErrorCheckCallback, updateAnglesQuery);
			Format(updateAnglesQuery, sizeof(updateAnglesQuery), "UPDATE rp_atms SET angle_z = '%.2f' WHERE uniqueId = '%s'", selfAngles[2], npcUniqueId);
			g_DB.Query(SQLErrorCheckCallback, updateAnglesQuery);
...
public Action cmdSpawnNpc(int client, int args) {
	...
	Format(insertNpcQuery, sizeof(insertNpcQuery), "INSERT INTO `rp_atms` (`id`, `uniqueId`, `name`, `map`, `model`, `idle_animation`, `second_animation`, `third_animation`, `pos_x`, `pos_y`, `pos_z`, `angle_x`, `angle_y`, `angle_z`, `type`, `flags`, `special_flags`, `enabled`, `created_by`, `timestamp`) VALUES (NULL, '%s', '', '%s', 'models/props_unique/atm01.mdl', 'idle_subtle', '', '', '%.2f', '%.2f', '%.2f', '%.2f', '%.2f', '%.2f', 'normal', '', '', '1', '%s', CURRENT_TIMESTAMP);", uniqueId, mapName, pos[0], pos[1], pos[2], angles[0], angles[1], angles[2], createdBy);
	g_DB.Query(SQLErrorCheckCallback, insertNpcQuery);
	
	
	CreateNpc(uniqueId, "", "models/props_unique/atm01.mdl", "idle_subtle", "Wave", "", pos, angles, "normal", "", "", true);
...
public Action chatHook(int client, int args) {
	...
	if (g_eNpcEdit[client][nWaitingForModelName] && StrContains(text, "abort") == -1) {
		...
		Format(updateModelQuery, sizeof(updateModelQuery), "UPDATE rp_atms SET model = '%s' WHERE uniqueId = '%s'", text, npcUniqueId);
		...
		Format(updateAnimationQuery, sizeof(updateAnimationQuery), "UPDATE rp_atms SET idle_animation = '%s' WHERE uniqueId = '%s'", text, npcUniqueId);
		...
		Format(updateNameQuery, sizeof(updateNameQuery), "UPDATE rp_atms SET name = '%s' WHERE uniqueId = '%s'", text, npcUniqueId);
...
public void loadNpcs() {
	...
	Format(loadNpcsQuery, sizeof(loadNpcsQuery), "SELECT * FROM rp_atms WHERE map = '%s';", mapName);
...
public int typeChooserHandler(Menu menu, MenuAction action, int client, int item) {
	if (action == MenuAction_Select) {
		...
		Format(updateTypeQuery, sizeof(updateTypeQuery), "UPDATE rp_atms SET type = '%s' WHERE uniqueId = '%s'", cValue, g_iNpcList[id][gUniqueId]);
